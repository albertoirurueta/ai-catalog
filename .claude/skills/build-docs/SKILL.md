---
name: build-docs
description: Build a repository's existing Antora documentation site, setting it up first (via `setup-antora`) if it doesn't exist yet. Invoke as `/build-docs`. Does no code exploration and makes no content edits to any existing page — it only ensures the toolchain/module exist and runs the Antora build, unlike `update-docs`, which edits pages to reflect codebase changes. Use whenever the user just wants the docs site built/verified (e.g. to check it compiles, or to get the built HTML output) instead of updating its content.
model: haiku
---
...
- **None found**: there is no documentation module yet to build. Ask the user whether Antora documentation
  should be set up now (via the `setup-antora` skill) before continuing — don't assume either way.
  - **User agrees**: run the `setup-antora` skill (via the Agent tool, or directly via Skill if already in this
...
Check Node.js is usable (`node -v` / `npm -v`). If either fails, stop and point the user at `setup-antora`'s
...
- **Build fails**: read the Antora/Asciidoctor error (broken `xref:`, bad YAML, malformed AsciiDoc table,
  ...
  page content yourself; that's `update-docs`'s job, not this skill's.

## Step 4 — Report

State whether the module already existed or was just set up via `setup-antora`, whether the build succeeded or
