---
name: database-code-one-task-group
description: Implement every task in one database-tagged plan-group bucket by calling `database-code-one-task` once per task, one at a time, in the plan's order — always sequential, regardless of the group's `Parallelizable` verdict, since database changes (schema migrations, seed data, index changes, and the queries a task calls for) commonly carry real ordering dependencies even when a plan doesn't flag one explicitly. Once every task in the bucket has been attempted, checks off each completed task's (and its sub-tasks') box in `implementation_plan.md` — noting its queries generated/executed and that `database-report.md` was updated — leaves any blocked task unchecked with its blocker noted, and notifies the user of the bucket's outcome (including the standing reminder to verify `database-report.md` before deciding whether to commit it). Unlike `java-code-one-task-group`/`dotnet-code-one-task-group`, it captures no pre-change quality baseline and runs no bucket-wide validation pass of its own — it relies on `database-code-one-task` to handle each task's own correctness. Invoke as `/database-code-one-task-group <bucket text>`, passing every task/sub-task in the bucket, each with its own text, plus any relevant "Current code state" context. Used by `code-one-task-group`, which invokes this once per plan group for its database-tagged tasks.
model: sonnet
...
`database-code-one-task` one at a time and keeping `implementation_plan.md` current once the bucket finishes.
This is deliberately simpler than `java-code-one-task-group`/`dotnet-code-one-task-group`: it has no pre-change
quality baseline and no bucket-wide test/coverage/quality validation pass, since there is no single standardized
toolchain across database platforms the way Maven/dotnet provide for Java/.NET — `database-code-one-task` is
...
## Step 1 — Implement each task via `database-code-one-task`, one at a time

For every task in the bucket, in the plan's order, invoke:

```
Skill({skill: "database-code-one-task", args: "<the task's full text, including its sub-tasks and any relevant
...
`java-code-one-task-group`/`dotnet-code-one-task-group`, it never dispatches tasks to parallel agents, even when
...
Hand control back to the caller (`code-one-task-group`) with a summary covering the whole bucket: per task, the
