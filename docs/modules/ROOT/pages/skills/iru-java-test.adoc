= Java Test
:description: Runs a Java project's JUnit suite via Maven Surefire, scoped to a class, method, package pattern, or tag expression, then reports a pass/fail summary.
:keywords: iru-java-test, claude code skill, java, maven, junit, surefire, run unit tests, test selector
...
* xref:skills/iru-java-code-one-task-group.adoc[] — Step 3, once per task group's bucket, via the
  xref:agents/iru-gate-runner.adoc[`iru-gate-runner`] agent, to run the scoped tests for every class the bucket affected
  (falling back to `mvn test -Dtest=<TestClass>` directly if this skill is unavailable), plus a full-suite run.
...
https://github.com/albertoirurueta/ai-catalog/blob/main/.claude/skills/iru-java-test/SKILL.md[`SKILL.md` on GitHub] — the file this page was generated from.
