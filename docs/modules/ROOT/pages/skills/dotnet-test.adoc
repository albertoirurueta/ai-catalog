...
* xref:skills/dotnet-code-one-task-group.adoc[] — Step 3, once per task group's bucket, via the
  xref:agents/gate-runner.adoc[`gate-runner`] agent, to run the scoped tests for every type the bucket affected
  (falling back to `dotnet test --filter "FullyQualifiedName~<TestClass>"` directly if this skill is
  unavailable), plus a full-suite run.
