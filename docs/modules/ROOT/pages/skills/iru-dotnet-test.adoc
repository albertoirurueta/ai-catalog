= .NET Test
:description: Runs a .NET project's or solution's test suite via dotnet test, scoped to a class, method, namespace pattern, or trait, and reports pass/fail results.
:keywords: iru-dotnet-test, claude code skill, dotnet, csharp, unit testing, xunit, nunit, mstest, run unit tests
...
* xref:skills/iru-dotnet-code-one-task-group.adoc[] — Step 3, once per task group's bucket, via the
  xref:agents/iru-gate-runner.adoc[`iru-gate-runner`] agent, to run the scoped tests for every type the bucket affected
...
https://github.com/albertoirurueta/ai-catalog/blob/main/.claude/skills/iru-dotnet-test/SKILL.md[`SKILL.md` on GitHub] — the file this page was generated from.
